...
mineBlock miner parent = fst . mineTransactions miner parent
...
verifyChain = foldrM verifyBlock 0
...
verifyBlock b@(Block hdr txs) parentHash = do
    ...
    guard (0 == txFrom (coinbase hdr) && blockReward == txAmount (coinbase hdr))
    return (hash b)
...
validateReceipt r hdr =
    txrBlock r == hash hdr && verifyProof (txroot hdr) (txrProof r)

mineTransactions
    :: Miner -> Hash -> [Transaction] -> (Block, [TransactionReceipt])
mineTransactions miner parent txs =
    head $ filter (validNonce . blockHdr . fst) $ map go [0 ..]
  where
    coinbase :: Transaction
    coinbase = coinbaseTx miner
    txsTree :: Tree Transaction
    txsTree = buildTree $ coinbase : txs
    baseHdr :: BlockHeader
    baseHdr = BlockHeader { parent   = parent
                          , coinbase = coinbase
                          , txroot   = treeHash txsTree
                          , nonce    = 0
                          }
    header :: Nonce -> BlockHeader
    header n = baseHdr { nonce = n }
    receipt :: Block -> Transaction -> TransactionReceipt
    receipt b tx = TxReceipt
        { txrBlock = hash b
        , txrProof =
            fromMaybe (error "mineTransactions: TransactionReceipt not found")
                $ buildProof tx txsTree
        }
    go :: Nonce -> (Block, [TransactionReceipt])
    go n =
        let block :: Block
            block = Block { blockHdr = header n, blockTxs = txs }
        in  (block, map (receipt block) txs)
...
pprHeader self@(BlockHeader parent cb txroot nonce) = pprV
    [ p ("hash", VH $ hash self)
    , p ("parent", VH parent)
    , p ("miner", VH $ txTo cb)
    , p ("root", VH txroot)
...
pprBlock (Block header txs) =
    pprHeader header . showString "\n" . pprTxs (coinbase header : txs)

pprTx :: Transaction -> ShowS
pprTx tx@(Tx from to amount) = pprH
    [ showString "Tx#"
    , showsHash (hash tx)
    , p ("from", VH from)
    , p ("to", VH to)
